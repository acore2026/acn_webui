...
class MOQVideoSubscriber:
    ...
    async def subscribe_to_track(
        ...
        # FIX: match publisher's namespace format (single element with "/" prefix)
        # Publisher uses single element: ["/namespace_string"]
        # So subscriber must also use single element format with "/" prefix
        if isinstance(namespace, list):
            ns_str = "/".join(namespace)
        else:
            ns_str = namespace
        # Add "/" prefix if missing (publisher always uses "/" prefix)
        if not ns_str.startswith("/"):
            ns_str = "/" + ns_str
        full_track_name = FullTrackName(
            namespace=[ns_str.encode()],
